Useful
https://www.percona.com/files/presentations/percona-live/nyc-2012/PLNY12-creating-full-text-based-services-sphinx-and-mysql.pdf
...
source myapp_source
{
	...
	sql_query		= \
		SELECT ID,Title from SiteTree_Live
...
## Indexing
Logon to sphinx box  and execute ./indexer.sh

Then run rotate, not sure why
...
## Query
```
mysql --host sphinx --port 9306

mysql> select ID from myapp_index where match('About');
+------+
| id   |
+------+
|    2 |
...
SELECT * FROM app_index WHERE query='@title (About);mode=extended';

 SELECT * FROM YourSphinxTable WHERE query='@title (TITLE) @name (NAME);mode=extended'
 ;
